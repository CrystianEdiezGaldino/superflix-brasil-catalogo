...
const MovieContent = ({ movie, hasAccess }: MovieContentProps) => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-10 py-4 sm:py-6 md:py-8">
      <div className="grid grid-cols-1 md:grid-cols-[300px,1fr] gap-6 sm:gap-8">
        {/* Poster */}
        <div className="relative aspect-[2/3] rounded-xl overflow-hidden shadow-xl ring-1 ring-white/10 mx-auto md:mx-0 max-w-[240px] sm:max-w-[280px] md:max-w-none">
          {movie.poster_path ? (
            <img
          ...
          {/* Badge com avaliação */}
          <div className="absolute top-2 sm:top-3 right-2 sm:right-3 bg-black/70 backdrop-blur-sm px-2 py-1 rounded-full flex items-center gap-1">
        ...
        <div className="space-y-4 sm:space-y-6">
          {/* Sinopse */}
          <div className="bg-black/30 backdrop-blur-sm rounded-xl p-4 sm:p-5 shadow-lg">
            <h2 className="text-lg sm:text-xl md:text-2xl font-bold text-white mb-2 sm:mb-3 flex items-center">
              <span className="bg-netflix-red h-5 sm:h-6 w-1 mr-2 rounded-full"></span>
              Sinopse
            </h2>
