...
const AnimeRecommendations = ({ anime }: AnimeRecommendationsProps) => {
  if (!anime.recommendations?.results || anime.recommendations.results.length === 0) {
    return null;
  }
  ...
  return (
    <div className="px-6 md:px-10 mb-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
        {anime.recommendations.results.slice(0, 6).map((recommendation) => (
          <Link
