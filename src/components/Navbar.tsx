...
const Navbar = ({
  ...
  return <header className={`fixed top-0 w-full z-50 transition-colors duration-300 ${isScrolled ? "bg-black" : "bg-gradient-to-b from-black/80 to-transparent"}`}>
      <div className="container max-w-full px-4 py-3 flex items-center justify-between">
        ...
        <div className="flex items-center">
          <form onSubmit={handleSearchSubmit} className="relative">
            <div className="flex items-center">
              {isSearchOpen && <Input type="search" placeholder="Títulos, pessoas, gêneros..." value={query} onChange={e => setQuery(e.target.value)} className="mr-2 w-[200px] md:w-[300px] bg-black/80 border-gray-700 placeholder:text-gray-400 text-white" />}
              <Button type={isSearchOpen ? "submit" : "button"} onClick={!isSearchOpen ? toggleSearch : undefined} variant="ghost" size="icon" className="text-white">
                <Search size={20} />
              </Button>
            </div>
          </form>
