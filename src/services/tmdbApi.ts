...
export const fetchSeriesDetails = async (id: string) => {
  try {
    ...
    if (!response.ok) {
      console.error(`Error fetching series details: ${response.status}`);
      return null;
    }
    const data = await response.json();
    return data;
...
export const fetchSeriesSeasonDetails = async (id: string, seasonNumber: number) => {
  try {
    ...
    if (!response.ok) {
      console.error(`Error fetching season details: ${response.status}`);
      return null;
    }
    const data = await response.json();
    return data;
...
export const fetchSpecificAnimeRecommendations = async () => {
    try {
        ...
        if (!response.ok) {
          console.error(`Error fetching anime recommendations: ${response.status}`);
          return [];
        }
        const data = await response.json();
...
export const fetchPopularSeries = async (page = 1, itemsPerPage = 20) => {
  try {
    ...
    if (!response.ok) {
      console.error(`Error fetching popular series: ${response.status}`);
      return [];
    }
    const data = await response.json();
...
export const fetchAnime = async (page = 1, itemsPerPage = 20) => {
  try {
    ...
    if (!response.ok) {
      console.error(`Error fetching anime: ${response.status}`);
      return [];
    }
    const data = await response.json();
...
export const fetchKoreanDramas = async (page = 1, itemsPerPage = 20) => {
  try {
    ...
    if (!response.ok) {
      console.error(`Error fetching Korean dramas: ${response.status}`);
      return [];
    }
    const data = await response.json();
