...
export const fetchUpcoming = async (): Promise<(Movie | Series)[]> => {
  try {
    const [moviesResponse, seriesResponse] = await Promise.all([
      fetch(`${TMDB_API_URL}/movie/upcoming?api_key=${TMDB_API_KEY}&language=pt-BR`),
      fetch(`${TMDB_API_URL}/tv/on_the_air?api_key=${TMDB_API_KEY}&language=pt-BR`)
    ]);
