...
import { NETFLIX_ORIGINALS } from "@/data/netflixOriginals";

export async function fetchNetflixOriginals(limit: number = 50): Promise<MediaItem[]> {
  try {
    // Remove duplicates from movies and series
    const uniqueMovies = Array.from(new Map(NETFLIX_ORIGINALS.movies.map(movie => [movie.id, movie])).values());
    const uniqueSeries = Array.from(new Map(NETFLIX_ORIGINALS.series.map(series => [series.id, series])).values());
