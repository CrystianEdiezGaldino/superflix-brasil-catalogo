...
import { PARAMOUNT_ORIGINALS } from "@/data/paramountOriginals";

export async function fetchParamountOriginals(limit: number = 50): Promise<MediaItem[]> {
  try {
    // Remove duplicates from movies and series
    const uniqueMovies = Array.from(new Map(PARAMOUNT_ORIGINALS.movies.map(movie => [movie.id, movie])).values());
    const uniqueSeries = Array.from(new Map(PARAMOUNT_ORIGINALS.series.map(series => [series.id, series])).values());
    ...
    console.error('Error fetching Paramount+ originals:', error);
    return [];
  }
} 