...
import { PRIME_ORIGINALS } from "@/data/primeOriginals";

export async function fetchPrimeOriginals(limit: number = 50): Promise<MediaItem[]> {
  try {
    // Remove duplicates from movies and series
    const uniqueMovies = Array.from(new Map(PRIME_ORIGINALS.movies.map(movie => [movie.id, movie])).values());
    const uniqueSeries = Array.from(new Map(PRIME_ORIGINALS.series.map(series => [series.id, series])).values());
    ...
    console.error('Error fetching Prime Video originals:', error);
    return [];
  }
} 