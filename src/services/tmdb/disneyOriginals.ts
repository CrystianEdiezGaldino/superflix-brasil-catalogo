...
import { DISNEY_ORIGINALS } from "@/data/disneyOriginals";

const fetchDisneyOriginals = async (limit: number = 50): Promise<MediaItem[]> => {
  try {
    // Remove duplicates from movies and series
    const uniqueMovies = Array.from(new Map(DISNEY_ORIGINALS.movies.map(movie => [movie.id, movie])).values());
    const uniqueSeries = Array.from(new Map(DISNEY_ORIGINALS.series.map(series => [series.id, series])).values());
