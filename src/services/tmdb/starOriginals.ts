...
import { STAR_ORIGINALS } from "@/data/starOriginals";

const fetchStarOriginals = async (limit: number = 50): Promise<MediaItem[]> => {
  try {
    // Remove duplicates from movies and series
    const uniqueMovies = Array.from(new Map(STAR_ORIGINALS.movies.map(movie => [movie.id, movie])).values());
    const uniqueSeries = Array.from(new Map(STAR_ORIGINALS.series.map(series => [series.id, series])).values());
    ...
    console.error('Error fetching Star originals:', error);
    return [];
  }
};

export { fetchStarOriginals }; 